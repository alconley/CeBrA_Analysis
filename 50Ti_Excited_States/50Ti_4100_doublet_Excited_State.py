...
def hdtv_fit_to_root_guas(kev_per_bin,fit_filename):
    ...
    for i in range(len(cal_list)):
        ...
        Effeciency_Corrected_Yield = abs((cal_list[i][4])/CeBrA_Efficiency(cal_list[i][0]))
        ...
        hdtv_fit = [Energy,FWHM,Volume,Effeciency_Corrected_Yield]
...
for i in range(len(cebra_fits)):
    ...
    print(f"This is index: {i}. [Energy,Width,Volume,Effeciency Corrected Yield]] = {cebra_fit_parameters[i]}")
...
def I_gamma(relative_gamma_index, other_gamma_index):
    ...
    Eff_Corrected_Yield_j = cebra_fit_parameters[relative_gamma_index][3]
    Eff_Corrected_Yield_i = cebra_fit_parameters[other_gamma_index][3]
    
    I_gamma = Eff_Corrected_Yield_i/Eff_Corrected_Yield_j
...
print(f"50Ti: {x1_E} keV Excited state")

print("\n")
print(f"50Ti: 4172 keV Excited state")
print(f"I(gamma) = I(2616 keV) = {I_2618}")
print(f"I(gamma) = I(1497 keV) = {I_1497}")
print("\n")
print(f"50Ti: 4147 keV Excited state")
print(f"I(gamma) = I(1472 keV) = {I_1472}")
